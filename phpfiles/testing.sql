create or replace view Departing as (SELECT Station.location as Depart, Stop.departure_time, Reserves.Train_Number
...
SELECT Station.location as Arrive, Depart, departing.departure_time, Stop.arrival_time, Reserves.Train_Number, 
Departs_From, Arrives_At, Class, TRUNCATE(Total_Cost, 2) as Total_Cost, Number_Baggages, Passanger_Name  
FROM Reserves JOIN Reservation NATURAL JOIN Train_Route natural join Stop natural join Station JOIN Departing 
WHERE Reserves.Reservation_ID = Reservation.Reservation_ID AND  Reserves.Reservation_ID = "34343"  
AND Reservation.Cust_User = "newuser" AND Train_Route.train_number = Reserves.train_number 
AND Departing.Train_Number=Reserves.Train_Number AND Station.location=reserves.arrives_at order by Reserves.train_number
